...
-- Combina nombres de campos correctos + lógica simplificada (≤2%, "Service + revisión", "6 horas")
...
BEGIN
    ...
    LOOP
        ...
        -- ✅ LÓGICA CORREGIDA: ≤2% en cualquiera de los dos criterios
        ...
        -- ✅ LÓGICA SIMPLIFICADA: Si es crítico, usar valores uniformes
        IF es_critico THEN
            -- ✅ TIEMPO Y MOTIVO UNIFORME PARA TODOS
    ...
    AND trasladar_a = 'Taller'
    AND interno NOT IN (
        -- ✅ SUBQUERY CORREGIDO: usar ≤2% en lugar de ≤5%
        ...
        AND (
            ...
            (v.kilometraje_actual IS NOT NULL
             ...
             AND ((COALESCE(v.intervalo_cambio_aceite, 10000) - (v.kilometraje_actual - v.aceite_motor_km))::numeric / COALESCE(v.intervalo_cambio_aceite, 10000)::numeric) * 100 <= 2)
            OR
            -- Crítico por Horas (≤2%)
...
SELECT 'Función actualizada con lógica simplificada: ≤2%, "Service + revisión", "6 horas"' as mensaje;