...
export default function RegistroServicioOrdenesPage() {
  ...
  const generarDescripcionTrabajoOrdenes = (codigosOC: string, proveedoresTexto: string): string => {
    if (componentesSeleccionados.size === 0) return `Servicio según OCs: ${codigosOC}`
    ...
    if (tiposDeMantenimiento.length > 0) {
      return `Servicio según OCs ${codigosOC}: ${tiposDeMantenimiento.join(', ')}`
    } else {
      return `Servicio según OCs: ${codigosOC}`
    }
  }
