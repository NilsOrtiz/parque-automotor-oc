...
export default function ListaPendientesPage() {
  ...
  const [tiempoEstimadoCustom, setTiempoEstimadoCustom] = useState<string>('')
    ...
    try {
      const response = await fetch('/api/migrar-pendiente', {
        ...
        body: JSON.stringify({
          ...
          tiempo_estimado_custom: tiempoEstimadoCustom || undefined
        })
      })
      ...
      if (result.success) {
        ...
        setTiempoEstimadoCustom('') // Limpiar tiempo personalizado
        ...
        {showMigrationModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              ...
              {/* Campo de tiempo estimado personalizado */}
              <div className="mb-6">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  ⏱️ Tiempo Estimado (Opcional)
                </label>
                <div className="space-y-2">
                  <input
                    type="text"
                    value={tiempoEstimadoCustom}
                    onChange={(e) => setTiempoEstimadoCustom(e.target.value)}
                  ...
                  <p className="text-xs text-gray-500">
                    Si no especificas, se calculará automáticamente según el problema reportado
                  </p>
                </div>
              </div>
